...
function yze_ajax () {
  ...
  this.getIframe = function (url, loadedCallback, errorCallback, submittedCallback) {
    ...
    this.loadedCallback = loadedCallback
    this.errorCallback = errorCallback
    loadedCallback('<iframe marginheight=\'0\' frameborder=\'0\'  width=\'100%\'  height=\'100%\'  src=\'' + url + '\'></iframe>')
  }
  ...
  function modifyForm (data, isInIframe) {
    ...
    // 让后端的界面能通过事件回调vue中的回调
    form.get(0).addEventListener('yze_ajax.submitted', function (e) {
      submittedCallback(e.detail)
    })
    form.unbind('submit')

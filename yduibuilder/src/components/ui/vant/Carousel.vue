<template>
    <div :draggable='draggable'
      ...
      <template v-if="uiconfig.meta.custom?.showIndicator">
        <ol class="van-carousel-indicators">
      ...
      <div class="van-carousel-inner">
        <template v-if="!uiconfig.items?.length">
          <div class="van-carousel-item active">
        ...
        <template v-else>
          <div v-for="(subpage, index) in uiconfig.items" :key="index"
               :class="{'van-carousel-item': true, 'active': (!uiconfig.meta.custom?.activeIndex && index ==0) || uiconfig.meta.custom?.activeIndex === index}">
      ...
      <template v-if="uiconfig.meta.custom?.showControl">
        <a class="van-carousel-control-prev" type="button" :data-target="`#${uiconfig.meta.id}`" data-slide="prev">
          <span class="van-carousel-control-prev-icon" aria-hidden="true"></span>
        </a>
        <a class="van-carousel-control-next" type="button" :data-target="`#${uiconfig.meta.id}`" data-slide="next">
          <span class="van-carousel-control-next-icon" aria-hidden="true"></span>
        </a>
      </template>
    </div>
</template>
...
export default {
  ...
  setup (props: any, context: any) {
    ...
    const myCss = computed(() => {
      ...
      arr.push('van-carousel slide')
      if (props.uiconfig.meta.custom?.effect === 'crossfade') {
        arr.push('van-carousel-fade')
      }
      return arr
    })

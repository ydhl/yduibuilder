  ...
  <div v-if="selectedUIItemId" class="style-panel">
    ...
    <div class="style-body d-none">
      <div class="row">
        ...
        <div class="col-sm-9">
          <input type="text" v-model.trim.lazy="title" class="form-control form-control-sm" id="form-title">
        </div>
      </div>
      <div class="row">
        <label for="form-desc" class="col-sm-3 col-form-label text-end">{{ t('style.base.desc') }}</label>
        <div class="col-sm-9">
          <textarea v-model.trim.lazy="desc" class="form-control form-control-sm" id="form-desc"></textarea>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup (props: any, context: any) {
    ...
    const desc = info.computedWrap('desc', '', '')
    const uiID = computed({
      get () {
    ...
    const itemStyle = computed(() => {
      ...
      return defineAsyncComponent(
        () => new Promise((resolve) => {
          require([`@/components/sidebar/style/${info.selectedUIItem.value.type}.vue`], resolve)
        }))
    })
    return {
      ...info,

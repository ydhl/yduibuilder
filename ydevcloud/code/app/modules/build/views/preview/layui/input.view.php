...
class Input_View extends Preview_View {
    ...
    private function body_css() {
        $arr[] = 'layui-input layui-pl-1 layui-pr-1 layui-d-flex layui-align-items-center';
...
<?php
    ...
    public function build_ui()
    {
        ...
        if (@$this->data['meta']['custom']['wordCountVisible']){
            echo $this->indent(3) . "<div class='layui-ml-3 ml-3 align-items-center'><span class='word-count'>0</span>";
            if (@$this->data['meta']['custom']['maxLength']){
        ...
        if (@$this->data['meta']['custom']['clearButtonVisible']){
            echo $this->indent(3) . "<button type='button' class='layui-btn layui-btn-primary layui-btn-xs layui-border-0 layui-reset d-none'><i class='layui-icon layui-icon-close'></i></button>\r\n";
        }

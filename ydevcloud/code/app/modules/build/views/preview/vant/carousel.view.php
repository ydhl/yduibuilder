...
class Carousel_View extends ValueList_View {
    ...
    const {$myid} = {}; 
    const {$myid}_subpages = document.querySelectorAll("[data-target='{$myid}'].van-carousel-item");
...
TICK;
    ...
    for( const ui of {$myid}_uis){
        const id = ui.id;
        if (!id) continue;
        let elements = document.querySelectorAll(`#\${id} .van-carousel-indicator`);
        ...
        elements = document.querySelectorAll(`#\${id} .van-carousel-item`);

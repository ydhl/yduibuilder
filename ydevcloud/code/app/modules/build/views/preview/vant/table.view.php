...
class Table_View extends Preview_View {
    ...
    public function build_style($justSelf = true)
    {
        ...
        $style['[data-uuid='.$this->myId().'] tfoot'] = $this->footer_style();
        $style['[data-uuid='.$this->myId().'] thead'] = $this->header_style();
        $style['[data-uuid='.$this->myId().'] table'] = $this->table_style();
        return $style;
    }
    ...
    protected function table_class()
    {
        ...
        foreach ($cssArray as $cssKey => $item) {
            if ($cssKey=='backgroundTheme' && $this->backgroundTheme()){
                ...
                $newCss[] = $item;
            }else if($cssKey=='backgroundTheme'){
                $newCss[] = $item;
            }
        }
        return join(' ',$newCss);

<template>
    <div v-attr="attrs" :class="css" :style="style" data-root :data-index="iterateIndex" :data-value="myValue">
        <template v-for="(item, index) in items" :key="index">
          <div class="nav-item" @click="updateValue(index, item);y.emit($el, 'y-click', $event, myValue, item)"
          ...
          @focus="y.emit($el, 'y-focus', $event, myValue, item)">
            <a :class="[itemCss,index == myValueIndex ? checkedItemCss : '']" :style="`${itemStyle};${index==myValueIndex ? checkedItemStyle : ''}`" href="javascript:;">{{ valueList.getItemTitle(item) }}</a>
          </div>
        </template>
    </div>
</template>
